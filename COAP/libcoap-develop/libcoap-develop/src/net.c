...
void client_encrypt(unsigned char* data);
void server_encrypt(unsigned char* data);
//void client_decrypt(unsigned char *data);
//void server_decrypt(unsigned char *data);
...
void client_encrypt(unsigned char* data)
{
	...
	char serv_public[] = "-----BEGIN RSA PUBLIC KEY-----\n"
	"MIIBCgKCAQEAyHo7N3DAmXGY5ADLXxZkP9Q72rVQJCfsFUgaNOD6JshBj0+8ZrsK\n"
	"viDUtTaLyBFaeAA1eb236T3GQAugtz1I2e0pO1AI4/NoT9KXrNTm8PmF/XsOhRJ/\n"
	"xAP01q/PKjQSqtVX/eQUvVXezAHbfbPXFqVeei6qZcVO/2o6Ub+q6b1dngwalxQ9\n"
	"cVXPTJ+IA4lNw29R3SGlipX1ESpMj4RgQ6j5sq0jXaRyhTQgCttcLv6mNRUP3VTJ\n"
	"CuyV62Kg5qXdi2F94JkMwtVubMksrptYxUJdmgEtTWnBmkPBIAy5GvVfnFgrN6Li\n"
	"e8fIZB+7yW8qzdEKdxJ2znfvS5tKENit+QIDAQAB\n"
	"-----END RSA PUBLIC KEY-----\n";
	
	public = BIO_new_mem_buf(serv_public,-1);
	if(public==NULL)
		printf("key creat error");

	if(!PEM_read_bio_RSAPublicKey(public,&rsa,NULL,NULL))
	{
		printf("1pem read error\n");
		exit(1);
	}
	data_len = strlen((char *)data);

	if((encrypt_len = RSA_public_encrypt(data_len,(unsigned char*)data,encrypted,rsa,RSA_PKCS1_OAEP_PADDING))==-1)
	{
		printf("client encrypt error\n");
		printf("fail data : [%s]\n",data);
		printf("fail data_len %d\n",data_len);
		printf("fail encrypt_len %d\n",encrypt_len);
		exit(2);
	}
	memcpy(data,encrypted,encrypt_len);
	//printf("%s\n",data);
	//	printf("fail encrypt_len %d\n",encrypt_len);
...
void server_encrypt(unsigned char *data)
{
	...
	BIO *public;
	RSA *rsa = NULL;
	char clnt_public[] = "-----BEGIN RSA PUBLIC KEY-----\n"
	"MIIBCgKCAQEAuLQJMhpNRk7b4VUpZwoJzi21sGyWTd3EVF2Zrf5tcFRJcYzFseTB\n"
	"Io/Voa33UZpr6qwUMJ3IYWBkedqmrdGX3tcOe3cUljrcb5kAHoTlPJDpm5CAXMd2\n"
	"Rx2GQMTMAYm4U/jxQr8Lv2VD+/Q2u3cAEsmq1FLeAkNQLdgMyk8stlYFiFRPMXxk\n"
	"unj8zfxQzVG6+V5hZMADdpu5aCNQcb9J0FW/8bOlUm7JHNBAbfcYaeFi5C0Oaq8I\n"
	"LRm/ymLo0mGQGJYos5bUEPnKQa7HrasoIak14CvloL2CfRs+qyjnmV60+rooafXy\n"
	"tPjlBuPIxw20wZR0ZVEjsOoYDv96cvW1vwIDAQAB\n"
	"-----END RSA PUBLIC KEY-----\n";


	public = BIO_new_mem_buf(clnt_public,-1);
	if(public==NULL)
		printf("key creat error");

	if(!PEM_read_bio_RSAPublicKey(public,&rsa,NULL,NULL))
	{
		printf("pem read error\n");
		exit(1);
	}
	data_len = strlen((char *)data);
...
coap_session_send_pdu(coap_session_t *session, coap_pdu_t *pdu) {
  ...
  //int data_len,encrypt_len;
  //char data[2048];
  assert(pdu->hdr_size > 0);
  //memset(data,0,2048);
  //data_len = strlen((const char*)pdu->data);
//	memcpy(data,pdu->data,data_len);
	extern int who;
  	if(pdu->data)
	{
		if(who==0)//client
			client_encrypt(pdu->data);
		else//server
			server_encrypt(pdu->data);
		pdu->used_size += 256;
	}
...
//  encrypt_len = strlen((const char*)pdu->data);
 // memset(pdu->data,0,encrypt_len);
...

Xgdb/CX+klmEFjtu5ma0rhzlJFLUzeqLqhxLewW2BiSn0138IFmp
-----END RSA PRIVATE KEY-----

	private = BIO_new_mem_buf(clnt_private,-1);
	if(private==NULL)
		printf("key creat error");

	if(!PEM_read_bio_RSAPrivateKey(private,&rsa,NULL,NULL))
	{
		printf("2pem read error\n");
		exit(1);
	}
	data_len = strlen((char *)data);

	if((decrypt_len = RSA_private_decrypt(data_len,(unsigned char*)data,decrypted,rsa,RSA_PKCS1_OAEP_PADDING))==-1)
	{
		printf("client decrypt error\n");
		printf("fail data : [%s]\n",data);
		printf("fail data_len %d\n",data_len);
		printf("fail decrypt_len %d\n",decrypt_len);
		exit(2);
	}
	memcpy (data, decrypted, decrypt_len);
}
*/
static void
...
/*void server_decrypt(unsigned char *data)
{
	...
	BIO *private;
	RSA *rsa = NULL;
	char serv_private[] = "-----BEGIN RSA PRIVATE KEY-----\n"
	"MIIEogIBAAKCAQEAyHo7N3DAmXGY5ADLXxZkP9Q72rVQJCfsFUgaNOD6JshBj0+8\n"
	"ZrsKviDUtTaLyBFaeAA1eb236T3GQAugtz1I2e0pO1AI4/NoT9KXrNTm8PmF/XsO\n"
	"hRJ/xAP01q/PKjQSqtVX/eQUvVXezAHbfbPXFqVeei6qZcVO/2o6Ub+q6b1dngwa\n"
	"lxQ9cVXPTJ+IA4lNw29R3SGlipX1ESpMj4RgQ6j5sq0jXaRyhTQgCttcLv6mNRUP\n"
	"3VTJCuyV62Kg5qXdi2F94JkMwtVubMksrptYxUJdmgEtTWnBmkPBIAy5GvVfnFgr\n"
	"N6Lie8fIZB+7yW8qzdEKdxJ2znfvS5tKENit+QIDAQABAoIBACAnMn4m0E0L4hiX\n"
	"ZAbBIZz3Hg5qzAqweHXFVaarepNjxNgGUtIrJ4Ls4xCZSfvxhJAAgJMg8XEwHi7B\n"
	"bqpU4u1I0zcXFAXzUQSUYw0uLEfzzSClbuO63tECTW9tTQ2YwnGxu+YVgZoIPq57\n"
	"kcdsY4/XP/rapyuhZJUkZWwdEH1iP4veRMByTHOBGad4GC17tJszXjDhM1RrFIVp\n"
	"H0gBQi8OPVD3VpgEuKrhrB5ximkm4W91eDOoJZuX7vebnRCjUVQgC8PAykci+/ad\n"
	"zrry7ucuyvWA3pb5mT2h3J1d15CmdgOQL3h6zTtaPOfj5UNoU2tGBW/jjlbDpoU8\n"
	"92wunZ0CgYEA/m7pvMghUkTRlBfhhfYR9kUqSxbWis5MrXdimxJMfut0wiRZJMr8\n"
	"40vzg2H/zpgXtRxIzxXiqE0f0OQRFuP8muaYNPwkCVWPgq0CK4Jin1xEKrRlzUXd\n"
	"hglfBJv4liG+WOAm0g6idZb8ajoBgtzU3tgSp/h9u8lquCPRfoCS/DMCgYEAybZD\n"
	"QWq5nOA/CJsdPxLBEEGA4fFxEcgvooguBJx0Alzk7lao9PfX/Fh9/NYeSmA9zabE\n"
	"nRuDJVnU/PW4QQLShO2sR7QyCQGK6tuSjxLNfanVpMBY6CfjIKwF4MzK+Xv0TPOG\n"
	"AcfwSOSD2p4guivQqYsUnCuaKACkv3/3qissASMCgYAKaI0lTOaYD/qm3Uc7jLJ1\n"
	"aaTuR3PlX8UhP4YII/p/IrC3FHeVGry3q669jC/DytMFdK/B0Pphui34BKxyTEdU\n"
	"KuTFHh93RknFpcA1+XKBZGvhd1nGyPnj9tQtQxgrRBweVKUh6LlaqB+RVOgk61XF\n"
	"+/7fYjn6iaXU/JogW7AnowKBgBPtB1PNASz4EgH9oLP8XEoIznyDZ1IEXoBFx5X5\n"
	"BpS9LHhG40XfvdxVuYxicneSbkiEcuO6NghYiWu7sknpx3YOLHig2lh9aIWuzxmH\n"
	"F/mT72DpB19XoV424BXG5T4j+ownelNuttPcjxHnkQATF/fHn2+V9xKAjcXFFGgg\n"
	"/6XVAoGAS7vas5UU/TFn/7JPzW2GQHcqYONZ57hV8cYl69HK933GDS0zKZc+12No\n"
	"qbLLih0QcsIKxSKORlCqrZJKHWQewBT0PPMONzyz4H37u4ieRx+8endYIhmC4n9E\n"
	"J4cFk0I+PMZxS5BWD7/E5tteeKx/GE1oiK6INe542eaWaUkintg=\n"
	"-----END RSA PRIVATE KEY-----\n";

	private = BIO_new_mem_buf(serv_private,-1);
	if(private==NULL)
		printf("key creat error");
