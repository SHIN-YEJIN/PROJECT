...
void get_public(RSA **rsa,BIO **public);
...
/*void get_public(RSA **rsa,BIO **public)
{
	*public = BIO_new(BIO_s_file());
	*rsa = RSA_new();

	if(*public ==NULL)
	{
        printf("biokey creat erro");
		exit(1);
    }
	if(*rsa ==NULL)
	{
        printf("rsa creat erro");
		exit(1);
	}
    if(BIO_read_filename(*public,"serv_public.pem")<=0)
    {
       printf("read public.pem erro");
       exit(2);
    }
    if(!PEM_read_bio_RSAPublicKey(*public,rsa,NULL,NULL))
    {
        printf("rsa pem read error");
        exit(3);
    }
	if(*rsa ==NULL)
	printf("rsa erro");
...
coap_new_request(coap_context_t *ctx,
 ...
 // unsigned char *ciphertext; 
  ...
  if(length) {
   ...
   //  get_public(&rsa,&public);//read public key
     // ciphertext = (unsigned char*)malloc(RSA_size(rsa)+1);
    // length = RSA_public_encrypt(length,pdu->data,ciphertext,rsa,RSA_PKCS1_OAEP_PADDING);//encrypt
 //end
